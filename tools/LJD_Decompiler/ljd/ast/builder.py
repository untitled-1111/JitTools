...
report_removed_instructions = True
...
class _State:
    ...
    def _warp_in_block(self, addr):
        try:
            self.block_starts[addr]
        except:
            for proper_addr in self.block_starts:
                addr = proper_addr
        finally:
...
def _build_function_definition(prototype, header):
    ...
    node.upvalues = prototype.constants.upvalue_references
...
def _build_function_blocks(state, instructions):
    _eliminate_illegal_instructions(state, instructions)
    ...
    for block in state.blocks:
        ...
        while addr <= block.last_address:
            ...
            try:
                ...
                print('Warning: an error occured while building an \'{0}\' statement'.format(instruction.name))
                statement, line_marked_elements = None, None
...
def _is_instruction_illegal(state, instructions, addr):
    instruction = instructions[addr]
    opcode = instruction.opcode
    ...
    if opcode in _WARP_INSTRUCTIONS:
        ...
        if not len(instructions) > destination >= 0:
            return True
        elif instructions[destination].name == "UNKNW":
            return True

        if ins.ITERL.opcode <= opcode <= ins.JITERL.opcode:
            if instructions[addr - 1].opcode not in (ins.ITERC.opcode, ins.ITERN.opcode):
                return True
            elif instructions[addr - 1].A < 3:
                return True
    elif opcode <= ins.ISF.opcode:
        if addr + 1 != len(instructions):
            if instructions[addr + 1].opcode not in _WARP_INSTRUCTIONS ^ {ins.ISNEXT.opcode}:
                return True
        else:
            return True
    elif opcode in (ins.FNEW.opcode, ins.TDUP.opcode, ins.KSTR.opcode, ins.KCDATA.opcode, ins.USETS.opcode):
        if not 0 <= instruction.CD < len(state.constants.complex_constants):
            return True
    elif opcode == ins.KNUM.opcode or opcode == ins.USETN.opcode \
            or ins.ADDVN.opcode <= opcode <= ins.MODNV.opcode:
        if not 0 <= instruction.CD < len(state.constants.numeric_constants):
            return True
    elif (opcode in (ins.CALLMT.opcode, ins.CALLT.opcode, ins.KNIL.opcode) \
            or ins.RETM.opcode <= opcode <= ins.RET1.opcode \
            or opcode == ins.USETV.opcode) \
            and not 0 <= instruction.CD <= instructions[0].A + 1:
        return True
    elif opcode <= ins.LEN.opcode and instruction.CD_type in (ins.T_VAR, ins.T_LIT):
        if not instructions[0].A > instruction.CD >= 0:
            return True
    elif opcode == ins.TSETM.opcode:
        if instruction.A == 0:
            return True
    elif addr > 0 and ins.FUNCF.opcode <= opcode <= ins.FUNCCW.opcode:
        return True
    elif opcode in (ins.ILOOP.opcode, ins.JLOOP.opcode):
        return True
    elif opcode == ins.CAT.opcode:
        if instruction.A >= instructions[0].A \
                or instruction.B >= instructions[0].A \
                or instruction.CD >= instructions[0].A:
            return True
    elif opcode in (ins.ITERC.opcode, ins.ITERN.opcode):
        if addr + 1 != len(instructions):
            if not ins.ITERL.opcode <= instructions[addr + 1].opcode <= ins.JITERL.opcode:
                return True
        else:
            return True
...
def _eliminate_illegal_instructions(state, instructions):
    while True:
        addr = 1
        instructions_removed = 0

        while addr < len(instructions):
            if _is_instruction_illegal(state, instructions, addr):
                # if we meet an illegal instruction, we replace it with UNKNW so it gets ignored
                # todo: unknown instructions removing and jump destination fixing
                if report_removed_instructions:
                    print('Removed instruction: {0} {1} '.format(addr, instructions[addr].name) \
                            + (hasattr(instructions[addr], 'A') and str(instructions[addr].A) + ' ' or '') \
                            + (hasattr(instructions[addr], 'B') and str(instructions[addr].B) + ' ' or '') \
                            + (hasattr(instructions[addr], 'CD') and str(instructions[addr].CD) or ''))

                instructions[addr] = ins.UNKNW()
                instructions[addr].opcode = 256
...
def _blockenize(state, instructions):
    ...
    # Fix "repeat until true" encapsulated by another loop; it might cause an error though
    ...
    if len(last_addresses) == 0 or last_addresses[-1] != len(instructions) - 1:
    ...
    # Stitch weirdly split blocks with instructions that are supposed to be together in one block
    while len(state.blocks) > 0:
        stop = False

        for index in range(0, len(state.blocks)):
            last_block = state.blocks[-1]
            first_block = state.blocks[index]

            if last_block == first_block:
                stop = True
                break

            second_block = state.blocks[index + 1]

            if (instructions[first_block.last_address].opcode <= ins.ISF.opcode \
                    and instructions[second_block.first_address].opcode in (ins.JMP.opcode, ins.UCLO.opcode)) \
                    or (instructions[first_block.last_address].opcode in (ins.ITERC.opcode, ins.ITERN.opcode) \
                    and ins.ITERL.opcode <= instructions[second_block.first_address].opcode <= ins.JITERL.opcode):
                first_block.last_address = second_block.last_address
                del state.block_starts[second_block.first_address]
                state.blocks.pop(index + 1)

                if state.blocks[-1] == first_block:
                    stop = True
                    break

                for index in range(index + 1, len(state.blocks)):
                    state.blocks[index].index -= 1
...
def _build_string_constant(state, index):
    ...
    try:
        state.constants.complex_constants[index]
    except IndexError:
        node.value = 'unknown'
    else:
        node.value = str(state.constants.complex_constants[index])
...
def _fix_broken_repeat_until_loops(state, instructions):
    ...
    for i, instruction in enumerated_instructions:
        ...
        if instruction.opcode == ins.LOOP.opcode:
            ...
            try:
                instructions[loop_condition_addr]
            except IndexError:
                continue
            else:
