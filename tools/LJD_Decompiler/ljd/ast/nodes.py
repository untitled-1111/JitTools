...
class BinaryOperator:
    ...
    def _accept(self, visitor):
        if self.right != self and self.left != self:
            ...
            print('Warning: a node tried to visit itself.')
    ...
    def __str__(self) -> str:
        return "BinaryOperator[%s, %d, %s]" % (str(self.left), self.type, str(self.right))
...
class BaseList:
    ...
    def contains_id(self, node) -> bool:
        return any([id(node) == id(i) for i in self.contents])
...
class IdentifiersList(BaseList):
    def __init__(self):
        self.contents = []
...
class ExpressionsList(BaseList):
    def __init__(self):
        self.contents = []
...
class Identifier:
    ...
    def _key(self):
        return self.name, self.type, self.id, self.slot
