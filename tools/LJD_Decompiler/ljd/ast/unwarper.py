...
def _find_endest_end(expressions: List[_ExpressionInfo]):
    endest_end = expressions[0].end

    for exp in expressions[1:]:
        _, _, exp_end, *_ = exp.tuple()
...
def _unwarp_expressions_pack(blocks, pack: List[_ExpressionInfo], meta: _StatementBlockMeta):
    ...
    for i, expr in enumerate(reversed(pack)):
        ...
        try:
            ...
            # And exclude the dummy blocks - they interfere with determining which
            #  block comes after the other, leading to some weird errors.
            _unwarp_logical_expression(start, end, _strip_dummy_blocks(body))
        except (AssertionError, IndexError):
            if catch_asserts:
                setattr(start, "_decompilation_error_here", True)
                print("-- WARNING: Error occurred during decompilation.")
...
def _find_expressions(start, body, end, level=0, known_blocks=None) -> (List[_ExpressionInfo], List[_ExpressionInfo]):
    block = None
    ...
    slot_assignments = []
    expressions: List[_ExpressionInfo] = []
    unused = []
    ...
    while i < len(extbody):
        ...
        subs: List[_ExpressionInfo] = []
        subs_unused: List[_ExpressionInfo] = []
        ...
        if len(subs) != 0:
            ...
            for sub in subs:
                _, sub_start, sub_end, sub_slot, sub_slot_type, *_ = sub.tuple()
        ...
        if isinstance(block.warp, nodes.ConditionalWarp):
            ...
            if not isinstance(warp_ident, nodes.Identifier):
                continue

            if not (dst.slot == slot or slot == -1):  # Slot will be -1 if we haven't found a node to set this from yet
                continue
...
def _unwarp_expression(body, end, true, false):
    ...
    # If either the true or false blocks are dummy blocks, follow them to the actual block
    # they would otherwise point to.
    true = _resolve_dummy_block(true)
    false = _resolve_dummy_block(false)
...
def _get_target(warp, allow_end=False, include_dummy_block=False):
    # Dummy blocks are inserted by the mutator containing assignments coming from some
    # instructions. Ignore them here, using what they target instead.
    if not include_dummy_block:
        return _resolve_dummy_block(_get_target(warp, allow_end=allow_end, include_dummy_block=True))
...
def _get_and_remove_last_assignment_source(block):
    if len(block.contents) > 0:
        ...
        expression = nodes.Primitive()
        expression.type = nodes.Primitive.T_TRUE
...
def _resolve_dummy_block(block):
    """
    If 'block' is a dummy block, return it's target. Otherwise return 'block'.

    This is useful when comparing warp jump targets against blocks in an
    expression, since the existence of the dummy block makes no difference due
    to how the unwarper is setup, from when the dummy blocks didn't exist.
    """
    if block is None or not hasattr(block, "_is_dummy_assn_block"):
        return block

    return _resolve_dummy_block(block.warp.target)


def _strip_dummy_blocks(blocks):
    return [b for b in blocks if not hasattr(b, "_is_dummy_assn_block")]
...
def _is_inverted(warp, true, end):
    # Resolve away dummy blocks from the warp targets before comparing
    # them, since the dummy blocks have been stripped from the input.
    if isinstance(warp, nodes.UnconditionalWarp):
        return _resolve_dummy_block(warp.target) == end

    if _resolve_dummy_block(warp.false_target) == true:
        return True
    elif _resolve_dummy_block(warp.false_target) == end:
