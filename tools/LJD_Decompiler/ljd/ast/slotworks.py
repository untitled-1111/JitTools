...
class RefsProcessData:
    ...
    # Keep track of 'elimination serial numbers' - increment this each time we
    #  eliminate an assignment, and use it to prevent slots from biting us later
    inline_serial: int = 0
    inline_limits: Dict[int, int] = field(default_factory=dict)
...
def simplify_ast(ast, dirty_callback=None, dirty_callback_once=None):
    visitor = _SimplifyVisitor(dirty_callback=dirty_callback)
    traverse.traverse(visitor, ast)

    if visitor.modified and dirty_callback_once:
        dirty_callback_once()
...
def _eliminate_temporary(ast, slots, ignore_ambiguous=True, safe_mode=True, unwarped=False):
    ...
    _fill_refs(data, ignore_ambiguous and safe_mode, True, eliminate=True)
    ...
    # Disabled temprarily due to it causing illegal code generation.
    # This is caused by the thing that is being put into the constructor
    # being a slot instead of another constructor.
...
def _fill_refs(data: RefsProcessData, ignore_ambiguous=True, safe_mode=True, eliminate=False):
    ...
    # Go through the slots backwards, so we can do the fancy anti-reordering stuff
    # See doc/SideEffectTracing.md
    for info in reversed(data.slots):
        ...
        if is_massive:
            ...
            _fill_simple_refs(info, data, ignore_ambiguous, safe_mode, eliminate=eliminate)

    # Since we went through the slots backwards before, we have to re-sort them so they're in
    # the correct order, as per the comment at the top of the file
    _sort_slots(data.simple, lambda i: i[0])
    _sort_slots(data.massive, lambda i: i[1])
    _sort_slots(data.iterators, lambda i: i[0])
    _sort_slots(data.tables, lambda i: i[0])
...
def _recheck_unsafe_cases(ast, unsafe, ignore_ambiguous, safe_mode=True, unwarped=False):
    ...
    def _dirty_cb(node):
        ...
        new_slots, _ = _collect_slots(ast, unwarped=unwarped)
        if safe_mode:
    ...
    if unwarped:
        simplify_ast(ast, dirty_callback_once=lambda: _dirty_cb(ast))
    else:
        for block in blocks:
            simplify_ast(block, dirty_callback_once=lambda: _dirty_cb(block))
...
def _fill_massive_refs(info, data: RefsProcessData, ignore_ambiguous, safe_mode=True):
    ...
    if isinstance(src, nodes.Primitive):
        ...
        # The writer has some checks to prevent it from writing the same node
        # twice. Unfortunately KNIL-based assignments have one Constant object
        # shared between all it's slots. Thus we have to clone them to prevent
        # the nils from going missing if they get inlined.
        _fill_simple_refs(info, data, ignore_ambiguous, safe_mode, clone_src=True)
        return
...
def _fill_simple_refs(info: 'SlotInfo', data: RefsProcessData, ignore_ambiguous, safe_mode=True, clone_src=False,
                      eliminate=False):
    src = info.assignment.expressions.contents[0]

    src_is_table = isinstance(src, nodes.TableConstructor)
    src_is_slot = isinstance(src, nodes.Identifier) and src.type == nodes.Identifier.T_SLOT
    ...
    # Find the container of the assignment. This is required because we might otherwise turn
    # assignments into table constructor elements even when they're in different scopes, such
    # as the scoped_table_constructor test:
    #
    # local mytbl = {}
    # if gbl_1 then
    #   mytbl.abc = 123
    ...
    # gbl_2 = mytbl
    #
    # Since the assignment to mytbl.abc would be mistaken as part of the table constructor. This
    # will later be used to check elements are indeed from the correct container.
    assignment_container = info.references[0].path[-4]
    ...
    for ref in info.references[1:]:
        ...
        # If this is part of a different scope, mark it as not being a table constructor - see the
        # comment for assignment_container.
        if statement_is_assignment and is_element and ref.path[ref.path.index(statement) - 1] != assignment_container:
            all_ctor_refs = False
        ...
        if src_is_table and is_element and is_dst and all_ctor_refs:
            ...
            ref_src = copy.copy(src) if clone_src else None
            new_simple.append((info, ref, ref_src))
            all_ctor_refs = False

    # (note: this comment block massively predates slotfinder and the other recent work)
    ...
    # Check that all the observable actions have been invalidated. An assignment is considered
    #  eliminated if it's invalidated (that is, it's been inlined) AND it's serial is <= our serial.
    # The serial check prevents slot-to-slot assignments from making us inline stuff improperly.
    elimination_safe = False
    if nr_simple_cases == 1 and safe_mode:
        ref = info.references[-1]
        max_serial = data.inline_limits.get(ref.identifier.id, None)

        def is_eliminated(assn: nodes.Assignment):
            if not _is_invalidated(assn):
                return False
            if max_serial is not None:
                return getattr(assn, "_inline_serial") <= max_serial
            return True

        if ref.observable_actions is not None and all(is_eliminated(a) for a in ref.observable_actions):
            elimination_safe = True

        # Note we can safely eliminate slot-to-slot assignments since they're handled specially via
        # serials - see the side effect tracing document on slot-to-slot assignments.
        if src_is_slot:
            elimination_safe = True
            data.inline_limits[src.id] = data.inline_serial

    # Add the ref to the appropriate array, depending on whether or not it's safe to inline
    if elimination_safe or (not safe_mode and nr_simple_cases <= 2):
        # We can't safely eliminate assignments from slots now - we have to do them in order, see
        # the big comment at the top of the file. Instead, queue them up to be done in the right order.
        if eliminate and not src_is_slot:
    ...
    # It's fine if _eliminate_simple_cases decides not to eliminate the slot - we check both that
    # it's been eliminated and for it's serial.
    # (in the context of a slot being inlined past an assignment that was not inlined but has a lower serial)
    setattr(info.assignment, "_inline_serial", data.inline_serial)
    data.inline_serial += 1
...
def _eliminate_simple_cases(simple):
    for info, ref, src in simple:
        ...
        # If a slot was referenced as an upvalue, we can't eliminate it, because temporary slots
        # never get referenced as upvalues.
        if hasattr(info, 'ref_as_uv'):
            continue
...
def _sort_slots(slots, to_slot=None):
    def get_slot_id(item):
        slot = to_slot(item) if to_slot else item
        return slot.slot_id
...
class _MultresEliminator(traverse.Visitor):
    ...
    def leave_assignment(self, node):
        ...
        if isinstance(dst, nodes.MULTRES):
            ...
            if self._last_multres_value is None:
                return
...
class SlotReference:
    observable_actions: Optional[List[nodes.Assignment]] = None
    """
    A list of all the assignments (containing observable actions) that occurred between when this slot
    was set and this reference. If any observable actions occurred outside an assignment, this is None.
    This will only be non-None for the first non-assignment reference, and only if side effect tracing
    is enabled when the slots are collected, and only when using SlotFinder.
    See the SideEffectTracing document.
    """
...
class _SimplifyVisitor(traverse.Visitor):
    ...
    def visit_function_call(self, node: nodes.FunctionCall):
        ...
        if not isinstance(func, nodes.TableElement) or not isinstance(func.table, nodes.Identifier):
            ...
            # FIXME couldn't this potentially cause issues? Why would this ever occur?
