...
class SimpleLoopWarpSwapper(traverse.Visitor):
    ...
    def _create_dummy_block(block, src, dst):
        ...
        # Mark this as being a jump warp, since we're probably going to jump over the
        # true condition.
        new_block.warp = nodes.UnconditionalWarp()
        new_block.warp.type = nodes.UnconditionalWarp.T_JUMP
        ...
        # HACK
        # Set a flag on this block which will be used by the unwarper to ignore it in it's
        # current (seemingly quite brittle) expression unwarper.
        setattr(new_block, "_is_dummy_assn_block", True)
    ...
    def _swap_iterator_warps(blocks, end):
        ...
        if isinstance(start.warp, nodes.UnconditionalWarp) \
                and start.warp.type == nodes.UnconditionalWarp.T_JUMP \
                and start.warp.target == end:
            ...
            setattr(new_end_warp, 'fixed_target', new_start_warp.body)
    ...
    def _swap_numeric_loop_warps(blocks, end):
        ...
        if isinstance(start.warp, nodes.UnconditionalWarp) \
                and start.warp.type == nodes.UnconditionalWarp.T_FLOW \
                and start.warp.target == warp.body:
            end_addr = end.warp._addr
            start_addr = start.warp._addr
            ...
            setattr(new_end_warp, 'fixed_target', new_end_warp.target)
...
# Split up the assignments resulting in KNIL instructions into multiple regular assignments.
# Massive nil assignments have historically caused a large number of issues, with various operations
#  intended to process nils not applying (or applying incorrectly) to massive nils. Unlike the
#  other massives, we do need to aggressively process nils to make clean code.
# The downside to this is the input won't so perfectly match the output, but it will behave identically.
class MassiveNilAssignmentSplitter(traverse.Visitor):
    def visit_block(self, node: nodes.Block):
        # We can't easily modify the node contents while iterating over it, so build a list of
        #  all the massives then swap them out later.
        massive_nils = []
        for i, assn in enumerate(node.contents):
            if not isinstance(assn, nodes.Assignment):
                continue
            if len(assn.destinations.contents) == 1:
                continue

            assert len(assn.expressions.contents) == 1
            src = assn.expressions.contents[0]
            if not isinstance(src, nodes.Primitive) or src.type != nodes.Primitive.T_NIL:
                continue

            massive_nils.append((i, assn))

        # Go backwards through the assignments so the indices are still valid
        for i, assn in reversed(massive_nils):
            assert node.contents[i] == assn
            assns = []
            for dst in assn.destinations.contents:
                new_assn = nodes.Assignment()
                assns.append(new_assn)

                for name in ("_addr", "_line"):
                    setattr(new_assn, name, getattr(assn, name))

                new_assn.destinations.contents.append(dst)
                new_assn.expressions.contents.append(copy.copy(assn.expressions.contents[0]))

            node.contents[i:i+1] = assns
...
def pre_pass(ast):
    ...
    traverse.traverse(MassiveNilAssignmentSplitter(), ast)
