...
_FORMAT_NO_DESC = "{addr:3}\t[{line:3}]\t{name:<5}\t{a:3}\t{b}\t{c}"
...
def _write_instruction(writer, addr, line, instruction):
    global no_comments, max_comment_length
    ...
    if not no_comments:
        try:
            ...
            if max_comment_length and len(description) > max_comment_length:
                description = None
        except:
            pass

    byte_array = instruction.Bytecode.to_bytes(4, byteorder="little")
    writer.stream.fd.write("["+"".join(r'%02X ' % x for x in byte_array)[0:-1]+"]")

    writer.stream.write_multiline(description and _FORMAT or _FORMAT_NO_DESC,
                                  addr=addr,
                                  line=line,
...
def _write_function(writer, addr, line, instruction):
    try:
        ...
        return _write_instruction(writer, addr, line, instruction)
...
def _translate(writer, addr, value, attr_type):
    ...
    if attr_type == ins.T_VAR:
        ...
        try:
            prototype.constants.numeric_constants[value]
        except IndexError:
            return "unknown"
        ...
        try:
            prototype.constants.complex_constants[value]
        except IndexError:
            return "unknown"
