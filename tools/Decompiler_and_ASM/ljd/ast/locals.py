...
def deduce_automatic_locals(ast: nodes.FunctionDefinition):
    ...
    for slot in slots:
        for ref in slot.references:
            if slot.slot < 30000:
                _setup_auto_slot(ref.identifier, slot)
        ...
        if not hasattr(slot.assignment, 'type') or slot.is_pinned:
            continue

        slot.assignment.type = nodes.Assignment.T_LOCAL_DEFINITION

    traverse.traverse(_UpvalueMarker(slots), ast)


def _setup_auto_slot(ident: nodes.Identifier, slot: slotworks.SlotInfo):
    ...
    # Note we're using slot.slot and not ident.slot - this matters when in an upvalue, where
    # the identifier has the UV number while the slot has the original slot number
    if is_argument:
        name = "arg%d" % slot.slot
    else:
        name = "slot%d_a%d" % (slot.slot, ident.id)

    ident.type = nodes.Identifier.T_LOCAL
    ident.name = name


class _UpvalueMarker(traverse.Visitor):
    def __init__(self, slots):
        super().__init__()
        self._slots = dict([(s.references[0].identifier.id, s) for s in slots])
    ...
    def visit_identifier(self, node: nodes.Identifier):
        ...
        try:
            _setup_auto_slot(node, self._slots[node.id])
        except:
            print('Warning: an error occured while working with upvalues.')
