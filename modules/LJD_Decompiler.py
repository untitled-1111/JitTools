...
def ljd_decompiler_proto(path):
  with open(path, 'rb') as f:
      file_path_abs = os.path.abspath(path)
      output_file = f"{os.path.dirname(os.path.dirname(os.path.realpath(__file__)))}\\{os.path.splitext(os.path.basename(path))[0]} - JTools (LJ_DEC_PROTO).lua"
